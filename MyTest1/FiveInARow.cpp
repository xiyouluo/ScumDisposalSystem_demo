...
FiveInARow::FiveInARow(QWidget *parent) : QMainWindow(parent)
{
        ...
        for (int j = 0; j < SIZE; j ++ ) {
            ...
            if (withAI) {
                ...
                if (board[i][j] == Piece::PLAYER) {
                    buttons[i][j]->setIcon(QIcon(":/mypic/pwhite.png"));
                    buttons[i][j]->setIconSize(QSize(49, 49));
                }
                else if (board[i][j] == Piece::AI) {
                    buttons[i][j]->setIcon(QIcon(":/mypic/pblack.png"));
                    buttons[i][j]->setIconSize(QSize(49, 49));
                }
            }
    ...
    setStyleSheet("FiveInARow { background-image: url(:/mypic/seperate.png); }");
}
...
void FiveInARow::buttonClicked() {
    ...
    if (button->icon().isNull()) {
        Piece currentPiece;
        if (turn) {
            button->setIcon(QIcon(":/mypic/pwhite.png"));
            currentPiece = Piece::PLAYER1;
        }
        else {
            button->setIcon(QIcon(":/mypic/pblack.png"));
            currentPiece = withAI ? Piece::AI : Piece::PLAYER2;
        }

        button->setIconSize(QSize(49, 49));

        bool buttonFound = false;
        for (int i = 0; i < SIZE && !buttonFound; i++)
            for (int j = 0; j < SIZE && !buttonFound; j++)
                if (buttons[i][j] == button) {
                    board[i][j] = currentPiece;
                    buttonFound = true;
                }

        if (withAI && !turn) makeAIMove();
        else turn = !turn;

        if (checkWin(Piece::PLAYER1)) {
            // 1 == 白棋
            QMessageBox::information(this, "Five-In-A-Row", QString("他赢了"));
        ...
        else if (!withAI && checkWin(Piece::PLAYER2)) {
            QMessageBox::information(this, "Five-In-A-Row", QString("你赢了"));
